...
if(player == _unit) exitWith {hint "Vous ne pouvez pas vous diagnostiquer vous meme !"};
if(!isPlayer _unit) exitWith {hint "Ce n'est pas un humain"};
if((player distance (getMarkerPos "asile_marker") > 90)) exitWith {hint "Vous n'etes pas assez prêt de l'asile psychatrique"};
...
hint "Vous avez empecher un fou d'agir, vous avez gagné 500€";