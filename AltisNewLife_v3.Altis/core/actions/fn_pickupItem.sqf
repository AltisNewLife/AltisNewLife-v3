...
if((time - life_action_delay) < 2) exitWith {hint "Vous ne pouvez pas utiliser rapidement la touche action !"};
...
if(playerSide == west && _illegal != -1) exitWith
{
	...
	compte_banque = compte_banque + ((life_illegal_items select _illegal) select 1);
	deleteVehicle _obj;
